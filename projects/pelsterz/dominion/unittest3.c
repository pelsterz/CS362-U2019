  ...
  for (p = 0; p < numPlayer - 1; p++)
  {
    for (handCount = 1; handCount <= maxHandCount; handCount++)
    {
      ...
      for(i = 0; i < numPlayer; i++){
        if(i != p){
          temp = G.handCount[i];
          ambassadorCard(c, 1, 1, 0, &G, h, &bonus, p); // choice 2 (discard hand, +4 cards)
